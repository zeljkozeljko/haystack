...
def plot_error_inlier_noise(errors_mean_normal, errors_std_normal,
                            errors_mean_spherical, errors_std_spherical,
                            ambient_dim, intrinsic_dim, inlier_points, outlier_points,
                            outlier_spreads, inlier_spreads, inlier_noise_spreads,
    ...
    if y_index == 0:
            ...
            for l, outlier_spread in enumerate(outlier_spreads):
                for w, inlier_spread in enumerate(inlier_spreads):
                    for m, inlier_noise_spread in enumerate(inlier_noise_spreads):
                        ax[0].errorbar(inlier_points, errors_mean_normal[:,k,w,l,m], errors_std_normal[:,k,w,l,m])
                        ax[1].errorbar(inlier_points, errors_mean_spherical[:,k,w,l,m], errors_std_spherical[:,k,w,l,m])
                        legend_entries.append((n_outlier, outlier_spread, inlier_spread, inlier_noise_spread))
        ax[0].set_xlabel(r'Inlier points', fontsize=16)
        ax[1].set_xlabel(r'Inlier points', fontsize=16)
        ax[0].set_ylabel(r'$\lambda_{max}(P - \hat{P})$', fontsize=16)
        ax[1].set_ylabel(r'$\lambda_{max}(P - \hat{P})$', fontsize=16)
        ...
        for k, n_inlier in enumerate(inlier_points):
            for l, outlier_spread in enumerate(outlier_spreads):
                for w, inlier_spread in enumerate(inlier_spreads):
                    for m, inlier_noise_spread in enumerate(inlier_noise_spreads):
                        ax[0].errorbar(outlier_points, errors_mean_normal[k,:,w,l,m], errors_std_normal[k,:,w,l,m])
                        ax[1].errorbar(outlier_points, errors_mean_spherical[k,:,w,l,m], errors_std_spherical[k,:,w,l,m])
                        legend_entries.append((n_inlier, outlier_spread, inlier_spread, inlier_noise_spread))
        ax[0].set_xlabel(r'Outlier points', fontsize=16)
        ax[1].set_xlabel(r'Outlier points', fontsize=16)
        ax[0].set_ylabel(r'$\lambda_{max}(P - \hat{P})$', fontsize=16)
        ax[1].set_ylabel(r'$\lambda_{max}(P - \hat{P})$', fontsize=16)
        ...
        for k, n_outlier in enumerate(outlier_points):
            for l, outlier_spread in enumerate(outlier_spreads):
                for w, inlier_point in enumerate(inlier_point):
                    for m, inlier_noise_spread in enumerate(inlier_noise_spreads):
                        ax[0].errorbar(inlier_spreads, errors_mean_normal[w,k,:,l,m], errors_std_normal[w,k,:,l,m])
                        ax[1].errorbar(inlier_spreads, errors_mean_spherical[w,k,:,l,m], errors_std_spherical[w,k,:,l,m])
                        legend_entries.append((n_outlier, outlier_spread, inlier_point, inlier_noise_spread))
        ax[0].set_xlabel(r'Inlier spread', fontsize=16)
        ax[1].set_xlabel(r'Inlier spread', fontsize=16)
        ax[0].set_ylabel(r'$\lambda_{max}(P - \hat{P})$', fontsize=16)
        ax[1].set_ylabel(r'$\lambda_{max}(P - \hat{P})$', fontsize=16)
        ...
        for k, n_outlier in enumerate(outlier_points):
            for l, n_inlier in enumerate(inlier_points):
                for w, inlier_spread in enumerate(inlier_spreads):
                    for m, inlier_noise_spread in enumerate(inlier_noise_spreads):
                        ax[0].errorbar(outlier_spreads, errors_mean_normal[l,k,w,:,m], errors_std_normal[l,k,w,:,m])
                        ax[1].errorbar(outlier_spreads, errors_mean_spherical[l,k,w,:,m], errors_std_spherical[l,k,w,:,m])
                        legend_entries.append((n_outlier, n_inlier, inlier_spread, inlier_noise_spread))
        ax[0].set_xlabel(r'Outlier spread', fontsize=16)
        ax[1].set_xlabel(r'Outlier spread', fontsize=16)
        ax[0].set_ylabel(r'$\lambda_{max}(P - \hat{P})$', fontsize=16)
        ax[1].set_ylabel(r'$\lambda_{max}(P - \hat{P})$', fontsize=16)
        ...
        for k, n_outlier in enumerate(outlier_points):
            for l, n_inlier in enumerate(inlier_points):
                for w, inlier_spread in enumerate(inlier_spreads):
                    for m, outlier_spread in enumerate(outlier_spreads):
                        ax[0].errorbar(inlier_noise_spreads, errors_mean_normal[l,k,w,m,:], errors_std_normal[l,k,w,m,:])
                        ax[1].errorbar(inlier_noise_spreads, errors_mean_spherical[l,k,w,m,:], errors_std_spherical[l,k,w,m,:])
                        legend_entries.append((n_outlier, n_inlier, inlier_spread, outlier_spread))
        ax[0].set_xlabel(r'Inlier (Gaussian) noise variance', fontsize=16)
        ax[1].set_xlabel(r'Inlier (Gaussian) noise variance', fontsize=16)
        ax[0].set_ylabel(r'$\lambda_{max}(P - \hat{P})$', fontsize=16)
        ax[1].set_ylabel(r'$\lambda_{max}(P - \hat{P})$', fontsize=16)
        plt.legend(legend_entries, ncol = 2)
